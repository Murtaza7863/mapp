...
export function Layout() {
  ...
  return (
    <div className="app-shell mx-auto flex min-h-dvh max-w-lg flex-col">
      <header className="glass-panel app-header sticky top-0 z-40 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-zinc-500 hover:text-zinc-300 rounded-xl p-2 transition-colors"
            aria-label="Search"
          >
      ...
      <main className="app-main flex-1 overflow-y-auto px-4 pt-4 pb-2">
        <OfflineBanner />
        <InstallPrompt />
        <Outlet />
      </main>
      ...
      <nav className="nav-floating fixed z-40 rounded-[1.25rem]">
        <div className="flex justify-around px-1 py-1.5">
          {mainLinks.map(({ to, label, Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex min-h-[48px] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-2 text-[10px] font-medium transition-colors ${
          ...
          <button
            ...
            className="text-zinc-500 flex min-h-[48px] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-2 text-[10px] font-medium"
          >
